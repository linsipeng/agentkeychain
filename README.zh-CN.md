...
系统会打开另一个一次性本地表单，最多允许 10 次只读校验，10 次都不正确才失效。
密码不会进入聊天或 MCP 参数，也不会修改 Vault 或 macOS 钥匙串。
...
v0.6.2 —— 紧急安全版本：测试默认禁止访问宿主机钥匙串与生产 Vault；自定义
Vault 使用按路径隔离的钥匙串条目；`setup` 在任何覆盖前必须先验证密码。
v1.0 前仍可能有破坏性变更。
